...
Button({ ...props, variant: 'secondary' }, primary)`}),z({class:"flex flex-col py-8"},[De({class:"text-3xl font-bold tracking-tight border-b pb-2"},"Variants")]),s({title:"Primary Button",description:"This is a primary button.",preview:[c({variant:"primary"},"Click Me")],code:`
...
LoadingButton('With Icon')`})]),Kt=e=>{const t=new Date(e.valueOf()),a=(e.getDay()+6)%7;t.setDate(t.getDate()-a+3);const n=t.valueOf();return t.setMonth(0,1),t.getDay()!==4&&t.setMonth(0,1+(4-t.getDay()+7)%7),1+Math.ceil((n-t)/6048e5)},be=(e,t)=>{const a=new Date(t,0,4),n=a.getDay(),l=new Date(a);l.setDate(a.getDate()-(n+6)%7);const f=new Date(l);return f.setDate(l.getDate()+(e-1)*7),f},Zt=(e,t,a)=>{const n=new Date;return n.getDate()===e&&n.getMonth()===t&&n.getFullYear()===a},Xt=({day:e,week:t,month:a,year:n,currentWeek:l,currentDate:f,selectWeek:b})=>{const B=Zt(e,a,n),te=t.some(D=>(D==null?void 0:D.getDate())===f);return W({text:e||"",disabled:!e,class:`
            px-2 py-1 text-center rounded-md
            ${B?"bg-accent text-accent-foreground":""}
            ${te&&!B?"border border-accent":""}
            hover:bg-primary hover:text-primary-foreground
        `,click:()=>b(l)})},eo=(e,t)=>{const a=new Date(e,t,1).getDay(),n=new Date(e,t+1,0).getDate(),l=[];let f=[];for(let b=1-a;b<=n;b++){const B=new Date(e,t,b);f.push(b>0?B:null),(f.length===7||b===n)&&(l.push([...f]),f=[])}return l},to=({selectWeek:e})=>ot("month",(t,a,{data:n})=>{const{year:l,month:f,currentWeek:b,currentDate:B}=n,te=eo(l,f);return o({class:"grid grid-cols-8 gap-1 text-sm px-4 py-2"},[o({class:"text-xs text-center col-span-1 text-muted-foreground flex items-center"},"Week"),o({class:"grid grid-cols-7 col-span-7 text-center text-muted-foreground items-center"},["Su","Mo","Tu","We","Th","Fr","Sa"].map(D=>i({class:"px-1 py-1",text:D}))),...te.map((D,Pa)=>{const U=Kt(D.find(_=>_)||new Date(l,f,1));return o({class:"grid grid-cols-8 col-span-8 items-center"},[o({class:"font-medium text-center col-span-1 rounded-sm cursor-pointer",click:()=>e(U),text:`${U}`,onSet:["currentWeek",{"text-primary-foreground":U,"bg-primary":U}]}),o({class:"grid grid-cols-7 col-span-7 text-center"},D.map(_=>Xt({year:l,month:f,day:_?_.getDate():null,week:D,currentWeek:b,currentDate:B,selectWeek:e})))])})])}),xe=({label:e,click:t})=>c({class:`
...
})`})]),ma=e=>!e||isNaN(e)?null:e,ye=(e,t)=>{const a=e,n=16,l=2*Math.PI*n,f=a/100*l,b=`
