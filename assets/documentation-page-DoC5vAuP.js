...
new InlineNavigation({
    ...
    type: 'warning', // 'success', 'destructive', 'info', 'warning'
    duration: 5000 // in milliseconds, leave it blank for 4 seconds, infinite is also a value to prevent it from going away
})`})]),Dt=()=>a({class:"absolute h-full rounded-full bg-primary transition-all duration-150",style:"width: [[progress]]%;"}),At=x({render(){return a({class:"relative w-full h-4 rounded-full bg-muted"},[Dt()])},state(){return{progress:this.progress??0}},set(e){this.state.progress=e}}),$t=()=>l({title:"Progress Bars",description:"Progress bars are a great way to show the progress of a task or process."},[s({title:"Usage",description:"Import the card atom and use it in your components.",preview:[new At({cache:"progress",progress:20})],code:`import { ProgressBar } from "@components/atoms/progress-bar.js";
