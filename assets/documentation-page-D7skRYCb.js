...
}).open()`})]),bt=u(e=>({tag:"select",...e,class:`flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 ${e.class||""}`,onCreated(t){e.options&&fe.setupSelectOptions(t,e.options)}})),ht=y({state(){return{selectedDate:this.selectedDate??null,open:!1}},render(){const e=(o,{state:n})=>n.toggle("open"),t=o=>{this.state.selectedDate=o,this.state.open=!1};return s({class:"relative w-full max-w-[320px]"},[A({class:"flex items-center gap-2 w-full justify-between border border-input bg-background hover:bg-accent hover:text-accent-foreground rounded-md h-10 px-4 py-2",click:e},[b({onState:["selectedDate",o=>o?be.format("standard",o):"Pick a date"]}),h({html:m.calendar.days})]),s({class:"absolute mt-1 z-10 bg-background rounded-md shadow-lg",onState:["open",o=>o?new G({selectedDate:this.state.selectedDate,selectedCallBack:t}):null]})])}}),gt=[{value:"next.js",label:"Next.js"},{value:"sveltekit",label:"SvelteKit"},{value:"nuxt.js",label:"Nuxt.js"},{value:"remix",label:"Remix"},{value:"astro",label:"Astro"}],xt=()=>l({title:"Inputs",description:"Inputs are essential for gathering user data in forms."},[a({title:"Usage",description:"Import the input atom and use it in your components. The input can be of various types: text, password, email, etc.",preview:[I({type:"text",placeholder:"Enter your text here...",change:e=>console.log(e.target.value)})],code:`import { Button } from '@components/atoms/form/input.js';
...
Input({
    type: 'text',
    placeholder: 'Enter your text here...'
})`}),a({title:"Tel Input",description:"This is a phone input.",preview:[he({})],code:`
import { Button } from '@components/atoms/form/input.js';

TelInput({
})`}),a({title:"Email Input",description:"This is a email input.",preview:[ge({})],code:`
import { Button } from '@components/atoms/form/input.js';
...
})`}),a({title:"Select",description:"This is a select input.",preview:[bt({options:gt,change:e=>console.log(e.target.value)})],code:`
import { Button } from '@components/atoms/form/input.js';
...
Textarea({
    placeholder: 'Enter your text here...'
})`}),a({title:"Date Picker",description:"@components/molecules/date-time/date-picker.js",preview:[new ht({selectedDate:"2022-01-01"})],code:`
import { Button } from '@components/atoms/form/input.js';
...
);`})]),O=(e,t,o)=>(o="Icons"+(o?"."+o:""),s({class:"flex flex-auto flex-col items-center gap-2 max-w-[150px] h-[8.5rem] cursor-pointer"},[s({class:"flex flex-auto w-full h-full justify-center items-center flex-wrap rounded-lg border bg-card text-base shadow-sm",click:()=>{navigator.clipboard.writeText(`${o}.${t}`)}},[h({html:e})]),s({class:"text-muted-foreground"},t)])),Z=(e,t,o)=>{const n=typeof e=="object";n&&t.push(c({class:"text-muted-foreground"},"path: "+o+".{icon}"));const p=[];if(t.push(s({class:"flex flex-auto flex-wrap gap-2 my-8"},p)),n===!1)p.push(O(e,o));else for(var g in e){if(e.hasOwnProperty(g)===!1)continue;const j=e[g];if(typeof j!="object"){p.push(O(j,g,o));continue}const X=`${o}.${g}`;Z(j,t,X)}},St=e=>{if(!e||typeof e!="object")return;const t=[];for(let[o,n]of Object.entries(e)){if(o==="parse")continue;t.push(q({id:o.toLowerCase(),class:"text-2xl font-bold"},Ce.titleCase(o))),Z(n,t,o)}return t},At=()=>l({title:"Icons",description:"The icons are from the Hericon library. They are SVG icons that can be used in your project. This icon set uses the outline style."},[M({href:"https://heroicons.com/",target:"_blank",class:"bttn link"},"link: Hericons"),a({title:"Icons",description:"This is an icon.",preview:[h({html:m.home})],code:`
import { Icons } from '@components/icons/icons.js';
