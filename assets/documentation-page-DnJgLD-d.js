...
])`})]),_=u((e,t)=>s({class:"py-4"},t)),eo=()=>l({title:"Tabs",description:"Displays a tab or a component that looks like a tab."},[n({title:"Button Tab",description:"This is a button tab. The buttons to not route.",preview:[new ne({class:"mt-4 max-w-[400px]",options:[{label:"Preview",value:"preview",layout:_([])},{label:"Code",value:"code",layout:_([])}]})],code:`
...
Tooltip({ content: 'This is a tooltip.', position: 'bottom' }, [
    Button('Hover me')
])`}),m({class:"text-muted-foreground text-sm mt-4"},"The tooltip position can be set to: top, top-right, top-left, bottom, bottom-right, bottom-left, left, right.")]),ae=u((e,t)=>{var o;return g({...e,html:(o=t[0])==null?void 0:o.textContent})}),q=(e,t,o)=>(o="Icons"+(o?"."+o:""),s({class:"flex flex-auto flex-col items-center gap-2 max-w-[150px] h-[8.5rem] cursor-pointer"},[s({class:"flex flex-auto w-full h-full justify-center items-center flex-wrap rounded-lg border bg-card text-base shadow-sm hover:bg-accent hover:text-accent-foreground",click:()=>{navigator.clipboard.writeText(`${o}.${t}`)}},[ae(e)]),s({class:"text-muted-foreground"},t)])),re=(e,t,o)=>{const a=typeof e=="object";a&&t.push(m({class:"text-muted-foreground"},"path: "+o+".{icon}"));const f=[];if(t.push(s({class:"flex flex-auto flex-wrap gap-2 my-8"},f)),a===!1)f.push(q(e,o));else for(var k in e){if(e.hasOwnProperty(k)===!1)continue;const A=e[k];if(typeof A!="object"){f.push(q(A,k,o));continue}const ie=`${o}.${k}`;re(A,t,ie)}},so=e=>{if(!e||typeof e!="object")return;const t=[];for(let[o,a]of Object.entries(e)){if(o==="parse")continue;t.push(se({id:o.toLowerCase(),class:"text-2xl font-bold"},Pe.titleCase(o))),re(a,t,o)}return t},no=()=>l({title:"Icons",description:"The icons are from the Hericon library. They are SVG icons that can be used in your project. This icon set uses the outline style."},[R({href:"https://heroicons.com/",target:"_blank",class:"bttn link"},"link: Hericons"),n({title:"Icons",description:"This is an icon.",preview:[g({html:r.home})],code:`
import { Icons } from '@components/icons/icons.js';
