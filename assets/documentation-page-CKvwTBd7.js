...
})`})]),Ht=u((e,t)=>ke({...e,class:"space-y-8 w-full max-w-lg"},t)),Ut=x({state(){return{error:null,value:this.defaultValue??""}},render(){const e=this.name,t=this.getId(`${e}`),{label:o,description:a,onValidate:r}=this,g=f=>{const v=r?r(f):null;this.state.error=v,this.state.value=f};return s({class:"space-y-4"},[zt([Ot({htmlFor:t},o),Gt({id:t,name:e,value:this.state.value,input:f=>g(f.target.value)},this.children),a&&Wt({id:`${t}-description`},a),s({onState:["error",f=>f&&Vt(f)]})])])}}),zt=u((e,t)=>s({...e,class:"space-y-2"},t)),Ot=u((e,t)=>ee({...e,class:"text-sm font-medium"},t)),Gt=u((e,t)=>s({...e},t)),Wt=u((e,t)=>d({...e,class:"text-sm text-muted-foreground"},t)),Vt=({children:e})=>d({class:"text-sm text-destructive"},e),Kt=e=>e.length<2?"Username must be at least 2 characters.":null,_t=()=>Ht([new Ut({name:"username",label:"Username",description:"This is your public display name.",onValidate:Kt},[F({placeholder:"e.g. email@address.com",required:!0})]),m({type:"submit",submit:e=>{e.preventDefault(),console.log("Form submitted")}},"Submit")]),qt=()=>c({title:"Forms",description:"Forms are used to collect user data and submit it to the server."},[n({title:"Usage",description:"To use the form component, import it from the components library and use it in your application.",preview:[_t()],code:`import { Button } from "@components/atoms/buttons/buttons.js";
...
export const ExampleTab = () => (
	new Tab({
        ...
        options: [
            ...
            `},t)])}),go=()=>c({title:"Tooltips",description:"This is a tooltip."},[n({title:"Usage",description:"Import the card atom and use it in your components.",preview:[s({class:"flex gap-2 flex-wrap"},[Y({content:"This is a tooltip."},[m("Hover me")])])],code:`import { Button } from "@components/atoms/buttons/buttons.js";
